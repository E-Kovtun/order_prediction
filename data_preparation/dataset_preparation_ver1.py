...
class OrderDataset:
    def __init__(self, data_folder, train_file, test_file, look_back, fix_material, current_info, predicted_value,
        ...
        if self.predicted_value == '42':
            self.target_feature = 'Time_less_42'
        ...
        if self.fix_material:
            ...
            self.constant_features_one_hot = ['Ship.to', 'PLZ', 'Material', 'MaterialGroup.1', 'MaterialGroup.2',
                                              'MaterialGroup.4']
        else:
    ...
    def add_time_difference(self, df):
        ...
        if self.newtime:
            df['Time_less_42'] = df['dt']
            for i in range(len(df['dt'])):
                df['Time_less_42'][i] = min(42, df['dt'][i])
    ...
    def window_combinations(self, df):
        ...
        if self.fix_material:
            ...
            for (ship_id, material_id) in proper_cases:
                chunks_relevant_indices.append(
                    df.loc[(df['Ship.to'] == ship_id) & (df['Material'] == material_id)].index)
        else:
    ...
    def construct_features(self):
        ...
        train_resulting_shape = (len(train_comb),
                                 ...
                                 self.current_info *
                                 ((self.target_feature == 'Amount_HL') * (1 + train_ohe_changing.shape[1] +
                                                                          train_vectorized_features.shape[1]) +
                                  (self.target_feature == 'dt') * (1 + train_vectorized_features.shape[1]) +
                                  (self.target_feature == 'Time_less_42') * (1 + train_vectorized_features.shape[1])))
        test_resulting_shape = (len(test_comb),
                                ...
                                self.current_info *
                                ((self.target_feature == 'Amount_HL') * (1 + test_ohe_changing.shape[1] +
                                                                         test_vectorized_features.shape[1]) +
                                 (self.target_feature == 'dt') * (1 + test_vectorized_features.shape[1]) +
                                 (self.target_feature == 'Time_less_42') * (1 + test_vectorized_features.shape[1])))
        ...
        for i, (ref_points, pred_point) in enumerate(train_comb):
            X_train[i, :] = np.concatenate([train_ohe_constant[ref_points[0], :].flatten(),
                                            ...
                                            train_numerical_features[pred_point, 0].flatten() if (
                                                        self.current_info and self.target_feature == 'Amount_HL') else np.empty(
                                                (0)),
                                            train_ohe_changing[pred_point, :].flatten() if (
                                                        self.current_info and self.target_feature == 'Amount_HL') else np.empty(
                                                (0)),
                                            train_vectorized_features[pred_point, :].flatten() if (
                                                        self.current_info and self.target_feature == 'Amount_HL') else np.empty(
                                                (0)),
                                            train_numerical_features[pred_point, 1].flatten() if (
                                                        self.current_info and self.target_feature == 'dt') else np.empty(
                                                (0)),
                                            train_vectorized_features[pred_point, :].flatten() if (
                                                        self.current_info and self.target_feature == 'dt') else np.empty(
                                                (0)),
                                            train_numerical_features[pred_point, 1].flatten() if (
                                                        self.current_info and self.target_feature == 'Time_less_42') else np.empty(
                                                (0)),
                                            train_vectorized_features[pred_point, :].flatten() if (
                                                        self.current_info and self.target_feature == 'Time_less_42') else np.empty(
                                                (0)),
        ...
        for i, (ref_points, pred_point) in enumerate(test_comb):
            X_test[i, :] = np.concatenate([test_ohe_constant[ref_points[0], :].flatten(),
                                           ...
                                           test_numerical_features[pred_point, 0].flatten() if (
                                                       self.current_info and self.target_feature == 'Amount_HL') else np.empty(
                                               (0)),
                                           test_ohe_changing[pred_point, :].flatten() if (
                                                       self.current_info and self.target_feature == 'Amount_HL') else np.empty(
                                               (0)),
                                           test_vectorized_features[pred_point, :].flatten() if (
                                                       self.current_info and self.target_feature == 'Amount_HL') else np.empty(
                                               (0)),
                                           test_numerical_features[pred_point, 1].flatten() if (
                                                       self.current_info and self.target_feature == 'dt') else np.empty(
                                               (0)),
                                           test_vectorized_features[pred_point, :].flatten() if (
                                                       self.current_info and self.target_feature == 'dt') else np.empty(
