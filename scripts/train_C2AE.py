...
from models.lstm_material import ClassificationNet
...
def train():
    prepared_folder = "../data/prepared/orders/"
    ...
    for rand_seed in tqdm(rand_seed_list):
        ...
        for epoch in range(1, num_epochs+1):
            ...
            for batch_ind, batch_arrays in tqdm(enumerate(train_dataloader), total=len(train_dataloader)):
                ...
                optimizer.zero_grad()
                optimizer.zero_grad()
                fx_x, fe_y, fd_z, fe_y_t = c2ae(batch_cat_arr, batch_mask_cat, batch_num_arr, batch_id_arr,
            ...
            for batch_ind, batch_arrays in enumerate(valid_dataloader):
                batch_arrays = [arr.to(device) for arr in batch_arrays]
                [batch_cat_arr, batch_current_cat, batch_dt_arr, batch_amount_arr, batch_id_arr] = batch_arrays # current_minus1_cat] = batch_arrays
