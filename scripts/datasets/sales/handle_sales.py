...
import os
import sys
sys.path.append("/NOTEBOOK/UHH/Repository/Repository_LSTM/Repository/")
from utils.split_data import split_dataset


def prepare_sales_dataset(init_folder, prepared_folder):
    data0 = pd.read_csv(init_folder+'sales_train.csv')
    items = pd.read_csv(init_folder+'items.csv')
