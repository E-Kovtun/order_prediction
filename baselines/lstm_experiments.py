...
class Baseline(torch.nn.Module):
    def __init__(self, data_shapes={'numerical' : [], 'categorical' : []}, model_params={
        'emb_size' : 32, 'num_out_size':64, 'classes' : cat_vocab_size + 1
    }): 
        super().__init__()
        ...
        self.cls_head = torch.nn.Sequential(
            torch.nn.Linear(model_params['num_out_size'], model_params['num_out_size']),
            self.act, 
    ...
    def forward(self, numerical, categorical):
        ...
        output, (hn, cn) = self.lstm(obj_embeddings)
        hn = hn[0]
        logits = self.cls_head(hn)[..., None]
