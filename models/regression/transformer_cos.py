...
class TransformerNet(nn.Module):
    def __init__(self, look_back, cat_vocab_size, id_vocab_size, amount_vocab_size, dt_vocab_size, emb_dim):
        ...
        self.linear1 = nn.Linear(3*emb_dim, emb_dim)
        self.relu = nn.ReLU()
    ...
    def forward(self, cat_arr, dt_arr, amount_arr, id_arr):
        ...
        x1 = self.linear1(x_encoder_output) # [batch_size, cat_vocab_size, emb_dim]
        x1 = self.relu(x1)
        x2 = self.linear2(x1) # [batch_size, cat_vocab_size, emb_dim]

        x_out = torch.abs(nn.CosineSimilarity(dim=2)(x2, x_cat_emb)) # [batch_size, emb_dim]
