...
class TransformerNet(nn.Module):
    def __init__(self, look_back, cat_vocab_size, id_vocab_size, amount_vocab_size, dt_vocab_size, emb_dim):
        ...
        self.linear_curr = nn.Linear(emb_dim, cat_vocab_size)
    ...
    def forward(self, cat_arr, dt_arr, amount_arr, id_arr, current_cat=None):
        ...
        x_encoder_input = torch.cat([x_id_emb, torch.cat([x_cat_emb, x_dt_emb, x_amount_emb], dim=2)], dim=1)  # [batch_size, cat_vocab_si
        ...
        x1 = self.bn1(x1)
        x1 = self.relu(x1)
        x_history_proj = self.linear2(x1) # [batch_size, cat_vocab_size]
